...
{X+}(*$HPPEMIT '#define IInterface IUnknown' *)
...
{X+}(*$HPPEMIT '  typedef bool *PBoolean;' *)
{X+}(*$HPPEMIT '  typedef PChar *PPChar;' *)
{X+}(*$HPPEMIT '  typedef double *PDouble;' *)
{X+}(*$HPPEMIT '  typedef wchar_t UCS4Char;' *)
{X+}(*$HPPEMIT '  typedef wchar_t *PUCS4Char;' *)
{X+}(*$HPPEMIT '  typedef DynamicArray<UCS4Char>  UCS4String;' *)
  (*$HPPEMIT '}' *)
  PLongint      = ^Longint;
...
{X+}var
{X+}  IsMemoryManagerSet : function : Boolean = MemoryManagerNotUsed;
...
{X+}// ************ UnregisterModule replacement ******************************
{X+}//
{X+}// Standard Delphi UnregisterModule uses
...
{X+}// implementation, "light" version of UnregisterModule
{X+}// is used by default. To use standard Delphi
{X+}// UnregisterModule method, allowing to flow execution control
{X+}// include reference to SysSfIni.pas
...
{X+}var
{X+}  UnregisterModule : procedure(LibModule: PLibModule) = UnregisterModuleLight;
{X+}// ************************************************************************
...
{X+}var
{X+}  InitUnitsProc : procedure( Table : PUnitEntryTable; Idx, Count : Integer )
{X+}        = InitUnitsLight;
{X+}procedure FinitUnitsHard;
{X+}var
{X+}  FinitUnitsProc : procedure = FInitUnitsLight;

{X+}// ***********************************************************************
{X+}// moved here from SysInit.pas - by advise of Alexey Torgashin - to avoid
{X+}//               creating of separate import block from kernel32.dll : }
...
{X+}// Moved here from below
{X+}const
{X+}  advapi32 = 'advapi32.dll';
{X+}  kernel = 'kernel32.dll';
{X+}  user = 'user32.dll';
{X+}  oleaut = 'oleaut32.dll';
{X+}
{X+}function GetProcessHeap; external kernel name 'GetProcessHeap';
{X+}function HeapAlloc; stdcall; external kernel name 'HeapAlloc';
{X+}function HeapReAlloc; stdcall; external kernel name 'HeapReAlloc';
{X+}function HeapFree; stdcall; external kernel name 'HeapFree';

{X+}// ***********************************************************************
{X+}// moved here from SysInit.pas - by advise of Alexey Torgashin - to avoid
{X+}//               creating of separate import block from kernel32.dll : }
{X+}function TlsAlloc: Integer; stdcall; external kernel name 'TlsAlloc';
{X+}function TlsFree(TlsIndex: Integer): Boolean; stdcall;
...
{X}function CmdShow: Integer;//GetCmdShow: Integer;
var
  SI: TStartupInfo;
begin
...
{X+}// ******  This code is from HeapMM.pas, (C) by Vladimir Kladov, 2001****
{X+}const
{X+}  HEAP_NO_SERIALIZE              = $00001;
{X+}  HEAP_GROWABLE                  = $00002;
{X+}  HEAP_GENERATE_EXCEPTIONS       = $00004;
{X+}  HEAP_ZERO_MEMORY               = $00008;
{X+}  HEAP_REALLOC_IN_PLACE_ONLY     = $00010;
{X+}  HEAP_TAIL_CHECKING_ENABLED     = $00020;
{X+}  HEAP_FREE_CHECKING_ENABLED     = $00040;
{X+}  HEAP_DISABLE_COALESCE_ON_FREE  = $00080;
{X+}  HEAP_CREATE_ALIGN_16           = $10000;
{X+}  HEAP_CREATE_ENABLE_TRACING     = $20000;
{X+}  HEAP_MAXIMUM_TAG               = $00FFF;
{X+}  HEAP_PSEUDO_TAG_FLAG           = $08000;
{X+}  HEAP_TAG_SHIFT                 =  16   ;
{X+}
{X+}{$DEFINE USE_PROCESS_HEAP}
{X+}
{X+}var
{X+}  HeapHandle: THandle;
{X+}  {* Global handle to the heap. Do not change it! }
{X+}
{X+}  HeapFlags: DWORD = 0;
{X+}//   Possible flags are:
{X+}//     HEAP_GENERATE_EXCEPTIONS - system will raise an exception to indicate a
{X+}//                              function failure, such as an out-of-memory
{X+}//                              condition, instead of returning NULL.
{X+}//     HEAP_NO_SERIALIZE - mutual exclusion will not be used while the HeapAlloc
{X+}//                       function is accessing the heap. Be careful!
{X+}//                       Not recommended for multi-thread applications.
{X+}//                       But faster.
{X+}//     HEAP_ZERO_MEMORY - obviously. (Slower!)
{X+}
{X+}//    Note from MSDN:
{X+}//    The granularity of heap allocations in Win32 is 16 bytes. So if you
{X+}//    request a global memory allocation of 1 byte, the heap returns a pointer
{X+}//    to a chunk of memory, guaranteeing that the 1 byte is available. Chances
{X+}//    are, 16 bytes will actually be available because the heap cannot allocate
{X+}//    less than 16 bytes at a time.
{X+}
{X+}function DfltGetMem(size: Integer): Pointer;
{X+}// Allocate memory block.
{X+}begin
{X+}  Result := HeapAlloc( HeapHandle, HeapFlags, size );
{X+}end;
{X+}
{X+}function DfltFreeMem(p: Pointer): Integer;
{X+}// Deallocate memory block.
{X+}begin
{X+}  Result := Integer( not HeapFree( HeapHandle, HeapFlags and HEAP_NO_SERIALIZE,
{X+}            p ) );
{X+}end;
{X+}
{X+}function DfltReallocMem(p: Pointer; size: Integer): Pointer;
{X+}// Resize memory block.
{X+}begin
{X+}  Result := HeapRealloc( HeapHandle, HeapFlags and (HEAP_NO_SERIALIZE and
{X+}         HEAP_GENERATE_EXCEPTIONS and HEAP_ZERO_MEMORY),
{X+}         // (Prevent using flag HEAP_REALLOC_IN_PLACE_ONLY here - to allow
{X+}         // system to move the block if necessary).
{X+}         p, size );
{X+}end;
{X+}
{X+}// ********************************end of HeapMM
...
{X}    GetMem:     DfltGetMem;
{X}    FreeMem:    DfltFreeMem;
{X}    ReallocMem: DfltReallocMem);

{X+}// by default, system memory allocation routines (API calls)
{X+}//    are used. To use Inprise's memory manager (Delphi standard)
{X+}//    call UseDelphiMemoryManager procedure.
{X+}const
{X+}  DelphiMemoryManager: TMemoryManager = (
{X+}    GetMem:     SysGetMem;
{X+}    FreeMem:    SysFreeMem;
{X+}    ReallocMem: SysReallocMem);
{X+}
{X+}procedure UseDelphiMemoryManager;
{X+}begin
{X+}  IsMemoryManagerSet := IsDelphiMemoryManagerSet;
{X+}  SetMemoryManager( DelphiMemoryManager );
{X+}end;
{X+}
...
{X+}        JA       @@short_up
{X+}
{X+}        LEA     ESI,[ESI+ECX-1] { point ESI to last byte of source     }
{X+}        LEA     EDI,[EDI+ECX-1] { point EDI to last byte of dest       }
{X+}        STD
{X+}
{X+}@@short_up:
{X+}        REP     MOVSB
...
{X+}        ADD     EDI, EAX
{X+}        REP     MOVSB
{X+}        *) // let's do it in other order - faster if data are aligned...
{X+}
{X+}        AND     ECX, 3
{X+}        LEA     ESI,[ESI+EAX-1]
{X+}        LEA     EDI,[EDI+EAX-1]
...
{X+}
{X+}        SAR     EAX, 2
{X+}        //JS    @@exit         // why to test this? but what does PC do?
{X+}        MOV     ECX, EAX
{X+}        MOV     EAX, 3
{X+}        SUB     ESI, EAX
{X+}        SUB     EDI, EAX
...
{X+}        CLD
{X+}        //JMP     @@exit
{X+}        DEC     ECX     // the same - loosing 2 tacts... but conveyer!
{X+}
{X+}@@long_up:
{X+}        SAR     ECX, 2
{X+}        JS      @@exit
...
{X+}procedure       FpuInit;
{X+}//const cwDefault: Word = $1332 { $133F};
{X+}asm
{X+}        JMP     _FpuInit
{X+}end;
{X+}
...
{X+}  Light : Boolean;
{X+}begin
{X+}  Light := @InitUnitsProc = @InitUnitsLight;
{X+}  while Idx < Count do
{X+}  begin
{X+}    P := Table^[ Idx ].Init;
{X+}    Inc( Idx );
{X+}    InitContext.InitCount := Idx;
{X+}    if Assigned( P ) then begin
{X+}{$IFDEF LINUX}
{X+}        CallProc(P, InitContext.Module^.GOT);
{X+}{$ENDIF}
{X+}{$IFDEF MSWINDOWS}
{X+}        TProc(P)();
{X+}{$ENDIF}
{X+}    end;
{X+}    if Light and (@InitUnitsProc <> @InitUnitsLight) then
{X+}    begin
{X+}      InitUnitsProc( Table, Idx, Count );
{X+}      break;
{X+}    end;
{X+}  end;
{X+}end;
...
{X+}var
{X+}  Count: Integer;
{X+}  Table: PUnitEntryTable;
{X+}  P: Pointer;
{X+}begin
{X+}  if InitContext.InitTable = nil then
{X+}        exit;
{X+}  Count := InitContext.InitCount;
{X+}  Table := InitContext.InitTable^.UnitInfo;
{X+}{$IFDEF LINUX}
{X+}  Inc(Cardinal(Table), InitContext.Module^.GOT);
{X+}{$ENDIF}
{X+}  while Count > 0 do
{X+}  begin
{X+}    Dec(Count);
{X+}    InitContext.InitCount := Count;
{X+}    P := Table^[Count].FInit;
{X+}    if Assigned(P) then
{X+}{$IFDEF LINUX}
{X+}        CallProc(P, InitContext.Module^.GOT);
{X+}{$ENDIF}
{X+}{$IFDEF MSWINDOWS}
...
{X+}begin
{X+}  try
{X+}    InitUnitsLight( Table, Idx, Count );
{X+}  except
{X+}    FInitUnitsHard;
{X+}    raise;
{X+}  end;
...
{X+}    end;
{X+}  except
{X+}    {X- rename: FInitUnits;  { try to finalize the others }
{X+}    FInitUnitsHard;
{X+}    raise;
{X+}  end;
{X+}end;
...
{X+}//  which leads to permanent use of all exception handler routines.
{X+}//  Let us make this by another way.
{X+}//**********************************************************************
{X-}//  try
{X-}//    while I < Count do
{X-}//    begin
...
{X+}//  but actually such code can be not used really.
{X+}//  So, implementation changed to avoid such references.
{X+}//
{X+}//  Either call UseErrorMessageBox or UseErrorMessageWrite
{X+}//  to provide error message output in GUI or console app.
{X+}//
{X+}
...
{X+}end;
{X+}
{X+}var ErrorMessageOutProc : procedure = DummyProc;
{X+}
{X+}procedure ErrorMessageBox;
{X+}begin
{X+}  MakeErrorMessage;
{X+}  if not NoErrMsg then
{X+}     MessageBox(0, runErrMsg, errCaption, 0);
{X+}end;
{X+}
{X+}procedure UseErrorMessageBox;
{X+}begin
{X+}  ErrorMessageOutProc := ErrorMessageBox;
{X+}end;
{X+}
{X+}procedure ErrorMessageWrite;
{X+}begin
{X+}  MakeErrorMessage;
{X+}  WriteLn(PChar(@runErrMsg));
{X+}end;
{X+}
{X+}procedure UseErrorMessageWrite;
{X+}begin
{X+}  ErrorMessageOutProc := ErrorMessageWrite;
{X+}end;
{X+}
{X+}procedure DoCloseInputOutput;
{X+}begin
{X+}  Close( Input );
{X+}  Close( Output );
{X+}  Close(ErrOutput);
{X+}end;
{X+}
{X+}var CloseInputOutput : procedure = DummyProc;
{X+}
{X+}procedure UseInputOutput;
{X+}begin
{X+}  if not assigned( CloseInputOutput ) then
{X+}  begin
{X+}    CloseInputOutput := DoCloseInputOutput;
{X+}    //_Assign( Input, '' );  was for D5 so - changed
{X+}    //_Assign( Output, '' ); was for D5 so - changed
{X+}    TTextRec(Input).Mode := fmClosed;
{X+}    TTextRec(Output).Mode := fmClosed;
{X+}    TTextRec(ErrOutput).Mode := fmClosed;
{X+}  end;
{X+}end;
{X+}

{X-}//procedure WriteErrorMessage;
{X-}//{$IFDEF MSWINDOWS}
{X-}//var
{X-}//  Dummy: Cardinal;
{X-}//begin
{X-}//  if IsConsole then
{X-}//  begin
{X-}//    with TTextRec(Output) do
{X-}//    begin
{X-}//      if (Mode = fmOutput) and (BufPos > 0) then
{X-}//        TTextIOFunc(InOutFunc)(TTextRec(Output));  // flush out text buffer
{X-}//    end;
{X-}//    WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), runErrMsg, Sizeof(runErrMsg), Dummy, nil);
{X-}//    WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), sLineBreak, 2, Dummy, nil);
{X-}//  end
{X-}//  else if not NoErrMsg then
{X-}//    MessageBox(0, runErrMsg, errCaption, 0);
{X-}//{$ENDIF}
{X-}//{$IFDEF LINUX}
{X-}//var
...
{X-}//      TTextIOFunc(InOutFunc)(TTextRec(Output));  // flush out text buffer
{X-}//  end;
{X-}//   __write(STDERR_FILENO, @runErrMsg, Sizeof(runErrMsg)-1);
{X-}//   c := sLineBreak;
{X-}//   __write(STDERR_FILENO, @c, 1);
{X-}//{$ENDIF}
{X-}//end;
...
{$IFNDEF PC_MAPPED_EXCEPTIONS}
{X-}//    UnsetExceptionHandler;
{X+}  UnsetExceptionHandlerProc;
{$ENDIF}
...
@@loop:
        ...
        DEC     ECX                             { if < 0: literal str           }
        JL      @@doneEntry
{X   LOCK} DEC     [EDX-skew].StrRec.refCnt        { threadsafe dec refCount       }
        JNE     @@doneEntry
        LEA     EAX,[EDX-skew].StrRec.refCnt    { if refCnt now zero, deallocate}
        CALL    _FreeMem
@@doneEntry:
...
{X-}//    tkWString: _WStrArrayClr(P^, elemCount);
{X+}tkWString: WStrArrayClrProc(P^, elemCount);
    tkVariant:
      while elemCount > 0 do
      begin
{X-}//        _VarClr(PVarData(P)^);
{X+}    VarClrProc(PVarData(P)^);
        Inc(Integer(P), sizeof(Variant));
        Dec(elemCount);
      end;
    tkArray:
...
@@WStringArray:
        MOV     EDX,ECX
{X-}//        CALL    _WStrArrayClr
{X+}    CALL    [WStrArrayClrProc]
        JMP     @@exit
@@Variant:
        MOV     EAX,EBX
        ADD     EBX,16
...
{X-}//procedure UnregisterModule(LibModule: PLibModule);
{X+}procedure UnregisterModuleSafely(LibModule: PLibModule);
var
  CurModule: PLibModule;
begin
...
{X+}procedure UnRegisterModuleLight( LibModule: PLibModule );
{X+}var
{X+}  P: PModuleUnloadRec;
{X+}begin
{X+}  P := ModuleUnloadList;
{X+}  while P <> nil do begin
{X+}    P.Proc(LibModule.Instance);
{X+}    P := P.Next;
{X+}  end;
{X+}{$IFDEF LINUX}
{X+}  InvalidateModuleCache;
{X+}{$ENDIF}
{X+}end;
...
{X+}{$IFDEF MSWINDOWS}
{X+}  {$IFDEF USE_PROCESS_HEAP}
{X+}    HeapHandle := GetProcessHeap;
{X+}  {$ELSE}
{X+}    HeapHandle := HeapCreate( 0, 0, 0 );
{X+}  {$ENDIF}
{X+}{$ENDIF}
...
{X-}//  RaiseExceptionProc := @RaiseException;
{X-}//  RTLUnwindProc := @RTLUnwind;
