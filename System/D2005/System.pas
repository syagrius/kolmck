...
type
  ...
  TVariantManager = record
    ...
    VarFromInt: procedure(var V: Variant; const Value: Integer; Range: Integer);
    ...
    OleVarFromInt: procedure(var V: OleVariant; const Value: Integer; Range: Integer);
 ...
 * Version: MPL 1.1
 *
 * The assembly implementation of procedure FillChar is subject to the
...
asm
        ...
        JL      @@done
  { LOCK} DEC     [EDX-skew].StrRec.refCnt        {NONthreadsafe dec refCount       }
        JNE     @@done
        PUSH    EAX
...
begin
  ...
  begin
    ...
    Inc(i);
  empty := True;
  if (s[i] = '$') and (Upcase(s[i+1]) = 'X') then
  begin
    if s[i] = '0' then
      Inc(i);
    Inc(i);
...
asm
        ...
        PUSH    EDX
        MOV     EAX,[EDX]
        CALL    [EAX] + VMTOFFSET IInterface._Release
        POP     EAX
@@1:
end;
{$ENDIF}
...
begin
  if Source = nil then
    Dest := nil
  else if Source.QueryInterface(IID, Temp) <> 0 then
