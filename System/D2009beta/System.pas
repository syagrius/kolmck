...
asm
        ...
        CMP     word ptr [EAX-skew].StrRec.elemSize,2
        JE      @@isUnicode
...
@@appendStr:
        PUSH    0                            // do this now because it usually won't be needed
        CMP     [ESI-skew].StrRec.elemSize,2
        JE      @@noTemp

        MOV     EDX,EAX                    // we need a real value here now, so put it in place
        MOV     EAX,ESP                    // reference it (var parameter)
        CALL    _UStrAsg                   // AddRef the string
        MOV     EAX,[ESP]
        MOV     ECX,[EAX-Skew].StrRec.length
...
@@theHardWay: // s(*EAX,ECX) := source1(EDX) + s(ECX)
        ...
        PUSH    0               // Prepare for a possible temp
        CMP     [EBX-Skew].StrRec.elemSize,2
        JE      @@source1IsUnicode

        MOV     EDX,EBX
        MOV     EAX,ESP
        CALL    _UStrAsg        // Assign to the temp and AddRef
        MOV     EAX,ESP
        CALL    EnsureUnicodeString  //  Convert it to Unicode
        MOV     EBX,[ESP]
...
@@source2IsUnicode:
        ...
        JZ      @@noTemp
        MOV     EAX,ESP
        CALL    _LStrClr        // Call _LStrClr directly since _UStrClr jumps to it

@@noTemp:
        POP     EAX             // Remove the temp from the stack
        POP     EAX             // EAX := Pop() // @s
        MOV     EDX,EDI         // EDX := result
        TEST    EDI,EDI
...
@@loop2: 
        ...
        MOV     EAX,[ESP+EBX*4+6*4]     // EAX := argN
        TEST    EAX,EAX
        JZ      @@2
        PUSH    0                             // Push a flag
        ...
        INC     ECX
        JLE     @@isConstant
   LOCK INC     [EAX-Skew].StrRec.refCnt      // This will ensure the "const" value isn't deallocted
@@isConstant:
        LEA     EAX,[ESP+EBX*4+7*4]
        CALL    EnsureUnicodeString
        MOV     EAX,[ESP+EBX*4+7*4]
        MOV     [ESP],1                       // Set clear flag
...
@@isUnicode:
        ...
        POP     EAX                     // Check if the string was reallocated
        TEST    EAX,EAX
        JZ      @@2
        LEA     EAX,[ESP+EBX*4+6*4]
        CALL    _LStrClr
@@2:
        DEC     EBX
        JNE     @@loop2
...
@@leftUnicode:
        CMP     [EDI-Skew].StrRec.elemSize,2
        JE      @@rightUnicode
